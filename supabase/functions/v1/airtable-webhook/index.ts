...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-airtable-sync-secret",
}
...
/**
 * Internal sync endpoint: call from Airtable Automations (HTTP request) or Automation scripts.
 * Body: { "airtableRecordId": "rec...", "reviewStatus": "circled" } OR { "clipId": "uuid", "reviewStatus": "circled" }
 * Auth: header X-Airtable-Sync-Secret must match env AIRTABLE_SYNC_SECRET (set in Supabase Edge secrets).
 */
serve(async (req) => {
  ...
  const secret = Deno.env.get("AIRTABLE_SYNC_SECRET")
  if (!secret || secret.length < 8) {
    return errorResponse(503, "AIRTABLE_SYNC_SECRET not configured", "misconfigured")
  }

  const provided = req.headers.get("x-airtable-sync-secret") ?? ""
  if (provided !== secret) {
  ...
  const airtableRecordId = typeof body.airtableRecordId === "string" ? body.airtableRecordId : ""
  const clipId = typeof body.clipId === "string" ? body.clipId : ""

  if (!airtableRecordId && !clipId) {
    return errorResponse(400, "Provide airtableRecordId or clipId", "missing_target")
  }
  ...
  if (!targetClipId && airtableRecordId) {
